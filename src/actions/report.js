...
import { actionStart, actionSuccess } from './utils/actionStatusMiddleware';

// Fetch report data
export const fetch = (uid, text, startDate, endDate) => dispatch => {
  dispatch(
    actionStart(types.REPORT_FETCH, {
      payload: {
        startDate,
        endDate
      }
  ...
  return new Promise(function(resolve, reject) {
    ref.once('value', function(snapshot) {
      const dispatchActionSuccess = entries => {
        dispatch(
          actionSuccess(types.REPORT_FETCH, {
            payload: {
              entries,
              startDate,
              endDate
